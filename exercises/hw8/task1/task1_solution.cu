...
__global__ void
naive_cuda_transpose(const int m, const double *const a, double *const c)
{
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stdout,
            "Total memory required per matrix is %lf MB\n",
    ...
    dim3 blocks(
        (size / THREADS_PER_BLOCK_X) + 1, (size / THREADS_PER_BLOCK_Y) + 1, 1);
