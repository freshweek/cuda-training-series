...
__global__ void kernel_a(float* x, float* y)
{
...
__global__ void kernel_c(float* x, float* y)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < N)
        y[idx] += 1;
...
int main()
{
    ...
    cublasSetVector(
        N, sizeof(h_x[0]), h_x, 1, d_x, 1);    // similar to cudaMemcpyHtoD
    cublasSetVector(
        N, sizeof(h_y[0]), h_y, 1, d_y, 1);    // similar to cudaMemcpyHtoD
    ...
    cudaGraphAddChildGraphNode(&libraryNode,
                               graph,
    ...
    cudaGraphAddKernelNode(&kernelNode2,
                           graph,
                           nodeDependencies.data(),
                           nodeDependencies.size(),
                           &kernelNode2Params);
