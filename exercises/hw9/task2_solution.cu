                             ...
                             const unsigned dsize)
{
    ...
    for (unsigned i = gidx; i < dsize; i += gridSize)
    {
        ...
        if (predicate_test(temp, remove_val))
        {
            ...
            for (unsigned j = 1;
                 (j - 1) < (g.group_index().x + (stride * gridDim.x));
                 j++)
...
int main()
{
    ...
    cudaLaunchCooperativeKernel(
        (void *)my_remove_if<mytype>, grid, block, args, 0, str);
    ...
    for (int i = 0; i < test_dsize; i++)
        ...
        {
            printf(
                "mismatch 1 at %d, was: %d, should be: %d\n", i, h_data[i], i);
            return 1;
        }
    ...
    for (int i = 0; i < val; i++)
        ...
        {
            printf(
                "mismatch 2 at %d, was: %d, should be: %d\n", i, h_data[i], i);
            return 1;
        }
