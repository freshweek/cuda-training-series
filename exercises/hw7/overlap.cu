...
__global__ void gaussian_pdf(const ft *__restrict__ x,
                             ft *__restrict__ y,
                             const ft mean,
                             const ft sigma,
                             const int n)
...
int main()
{
    ...
    gaussian_pdf<<<(ds + 255) / 256, 256>>>(
        d_x, d_y, 0.0, 1.0, ds);    // warm-up
    ...
    {    // depth-first launch
        cudaMemcpyAsync(d_x + FIXME,
                        h_x + FIXME,
                        (FIXME) * sizeof(ft),
                        cudaMemcpyHostToDevice,
                        streams[FIXME]);
        gaussian_pdf<<<((FIXME) + 255) / 256, 256, 0, streams[FIXME]>>>(
            d_x + FIXME, d_y + FIXME, 0.0, 1.0, FIXME);
    ...
    for (int i = 0; i < ds; i++)
    {
        if (h_y[i] != h_y1[i])
        {
            std::cout << "mismatch at " << i << " was: " << h_y[i]
                      << " should be: " << h_y1[i] << std::endl;
            return -1;
        }
    }
